...
module.exports = {
  ...
  PMClient: createModel('pm_clients', {
    user_id: DataTypes.UUID,
    pm_id: DataTypes.UUID,
    status: DataTypes.STRING,
    invested_amount: DataTypes.DECIMAL(15, 2),
    current_value: DataTypes.DECIMAL(15, 2),
    unrealized_pnl: DataTypes.DECIMAL(15, 2),
    account_number: DataTypes.STRING,
    risk_profile: DataTypes.STRING,
    joined_at: DataTypes.DATE
  }),
  PMHolding: createModel('pm_holdings', {
    client_id: DataTypes.UUID,
    stock_symbol: DataTypes.STRING,
    stock_name: DataTypes.STRING,
    quantity: DataTypes.DECIMAL(15, 4),
    avg_buy_price: DataTypes.DECIMAL(15, 4),
    current_price: DataTypes.DECIMAL(15, 4),
    current_value: DataTypes.DECIMAL(15, 2),
    unrealized_pnl: DataTypes.DECIMAL(15, 2),
    allocation_percent: DataTypes.DECIMAL(5, 2)
  }),
  PMInvoice: createModel('pm_invoices', {
    client_id: DataTypes.UUID,
    invoice_number: DataTypes.STRING,
  ...
  PMStrategy: createModel('pm_strategies', {
    pm_id: DataTypes.UUID,
  ...
  PMTradeOrder: createModel('pm_trade_orders', {
    pm_id: DataTypes.UUID,
    strategy_id: DataTypes.UUID,
    stock_symbol: DataTypes.STRING,
  ...
  RefundRequest: createModel('refund_requests', {
    ticket_id: DataTypes.UUID, // Can be null if subscription refund
    user_id: DataTypes.UUID,
  ...
  PayoutRequest: createModel('payout_requests', {
    user_id: DataTypes.UUID,
    entity_id: DataTypes.UUID,  // Missing field causing error
    entity_type: DataTypes.STRING,  // Missing field causing error
    amount: DataTypes.DECIMAL(15, 2),
    payment_method: DataTypes.STRING,
  ...
  FundTransaction: createModel('fund_transactions', {
    user_id: DataTypes.UUID,
    fund_id: DataTypes.UUID,
    amount: DataTypes.DECIMAL(15, 2),
    ...
    currency: DataTypes.STRING,
    source: DataTypes.STRING,
    completed_at: DataTypes.DATE  // Added for order by clause
  }),
  ModerationLog: createModel('moderation_logs', {
    moderator_id: DataTypes.UUID,
    user_id: DataTypes.UUID, // Offending user
    action: DataTypes.STRING,
    target_type: DataTypes.STRING,
    target_id: DataTypes.UUID,
    violation_type: DataTypes.STRING,
    message_content: DataTypes.TEXT,
    reason: DataTypes.TEXT,
    severity: DataTypes.STRING,
    status: DataTypes.STRING,
    admin_reviewed: { type: DataTypes.BOOLEAN, defaultValue: false }
  }),
  ...
  PromoCode: createModel('promo_codes', {
    code: { type: DataTypes.STRING, unique: 'promo_codes_code_uk', allowNull: false },
    discount_percent: DataTypes.INTEGER, // If percentage-based
    discount_amount: DataTypes.DECIMAL(10, 2), // If fixed amount
    max_uses: DataTypes.INTEGER, // Global limit
    uses_count: { type: DataTypes.INTEGER, defaultValue: 0 },
    expires_at: DataTypes.DATE,
    is_active: { type: DataTypes.BOOLEAN, defaultValue: true },
    applicable_to: DataTypes.JSON, // Array of plan IDs or 'all'
    min_order_amount: DataTypes.DECIMAL(10, 2),
    one_time_per_user: { type: DataTypes.BOOLEAN, defaultValue: false }, // If true, user can use only once
    applies_to_renewal: { type: DataTypes.BOOLEAN, defaultValue: true }, // If true, discount applies to renewals too
    created_by: DataTypes.UUID // Admin ID who created
  }),
  SubscriptionTransaction: createModel('subscription_transactions', {
    subscription_id: DataTypes.UUID,
    user_id: DataTypes.UUID, // Link to user for easier querying
    invoice_id: DataTypes.UUID, // Link to invoice
    amount: DataTypes.DECIMAL(10, 2),
    status: DataTypes.STRING, // success, failed, pending, refunded, partially_refunded
    payment_method: DataTypes.STRING,
    transaction_id: DataTypes.STRING, // Razorpay Payment ID
    razorpay_order_id: DataTypes.STRING,
    coupon_used: DataTypes.STRING, // Code of coupon used
    discount_applied: DataTypes.DECIMAL(10, 2),
    transaction_type: { type: DataTypes.STRING, defaultValue: 'charge' }, // charge, refund
    date: { type: DataTypes.DATE, defaultValue: DataTypes.NOW },
    metadata: DataTypes.JSON // Store extra razorpay validation info
  }),
  Subscription: createModel('subscriptions', {
    user_id: DataTypes.UUID,
    plan_id: DataTypes.UUID,
    status: DataTypes.STRING, // active, cancelled, expired, past_due
    ...
    // Autopay and cancellation tracking
    ...
    billing_cycle: DataTypes.STRING, // 'monthly' or 'annually'
  ...
  SubscriptionPlan: createModel('subscription_plans', {
    name: DataTypes.STRING,
    price: DataTypes.DECIMAL(10, 2),  // Keep for backward compatibility
    price_monthly: DataTypes.DECIMAL(10, 2),  // NEW
    price_annually: DataTypes.DECIMAL(10, 2),  // NEW
    interval: DataTypes.STRING,
    features: DataTypes.JSON,
    description: DataTypes.TEXT,
    is_active: DataTypes.BOOLEAN,
    is_system_plan: DataTypes.BOOLEAN,  // NEW - prevents deletion of core plans
    trial_days: DataTypes.INTEGER,
    advisor_id: DataTypes.UUID,
    finfluencer_id: DataTypes.UUID
  }),
  Invoice: createModel('invoices', {
    invoice_number: DataTypes.STRING, // INV-YYYYMM-0001
    user_id: DataTypes.UUID,
    subscription_id: DataTypes.UUID, // Can be null if one-off
    plan_name: DataTypes.STRING,
    amount: DataTypes.DECIMAL(10, 2), // Subtotal
    discount: { type: DataTypes.DECIMAL(10, 2), defaultValue: 0.00 },
    tax: { type: DataTypes.DECIMAL(10, 2), defaultValue: 0.00 },
    total_amount: DataTypes.DECIMAL(10, 2), // Grand Total
    currency: { type: DataTypes.STRING, defaultValue: 'INR' },
    status: { type: DataTypes.STRING, defaultValue: 'pending' }, // paid, pending, void, refunded
    payment_method: DataTypes.STRING,
    transaction_id: DataTypes.STRING,
  ...
  AdvisorPost: createModel('advisor_posts', {
    advisor_id: DataTypes.UUID,
    content: DataTypes.TEXT,
    media_urls: DataTypes.JSON,
    status: DataTypes.STRING,
  ...
  Poll: createModel('polls', {
    ...
    created_by: DataTypes.UUID, // Standardized field for poll creator
    created_by_role: DataTypes.STRING, // User role who created poll
    question: DataTypes.STRING, // Legacy field
    title: DataTypes.STRING, // New field - poll title
    description: DataTypes.TEXT, // Poll description
    stock_symbol: DataTypes.STRING, // Stock ticker symbol
    poll_type: DataTypes.STRING, // Type: buy_sell_hold, price_prediction, etc.
    target_price: DataTypes.DECIMAL(10, 2), // Target price for predictions
    options: { type: DataTypes.JSON, defaultValue: [] }, // Poll options array
    votes: { type: DataTypes.JSON, defaultValue: {} }, // Vote counts object
    total_votes: { type: DataTypes.INTEGER, defaultValue: 0 },
    // Legacy vote count fields for backward compatibility
    bullish_votes: { type: DataTypes.INTEGER, defaultValue: 0 },
    bearish_votes: { type: DataTypes.INTEGER, defaultValue: 0 },
    neutral_votes: { type: DataTypes.INTEGER, defaultValue: 0 },
    buy_votes: { type: DataTypes.INTEGER, defaultValue: 0 },
    sell_votes: { type: DataTypes.INTEGER, defaultValue: 0 },
    hold_votes: { type: DataTypes.INTEGER, defaultValue: 0 },
    ...
    is_premium: { type: DataTypes.BOOLEAN, defaultValue: false }, // Premium poll flag
    is_active: { type: DataTypes.BOOLEAN, defaultValue: true }, // Active status
    image_url: DataTypes.STRING, // Poll image
    confidence_score: DataTypes.INTEGER, // Confidence level
    creation_source: DataTypes.STRING, // Source: admin_panel, chat, etc.
    created_by_admin: { type: DataTypes.BOOLEAN, defaultValue: false }, // Created by admin flag
    is_boosted: { type: DataTypes.BOOLEAN, defaultValue: false }, // Boosted poll flag
    boost_expires_at: DataTypes.DATE, // When boost expires
    boosted_by: DataTypes.UUID // User who boosted the poll
  }),
  ...
  MessageReadReceipt: createModel('message_read_receipts', {
    message_id: DataTypes.UUID,
    user_id: DataTypes.UUID,
  ...
  ChatRoom: createModel('chat_rooms', {
    ...
    description: DataTypes.TEXT,
    stock_symbol: DataTypes.STRING,
    room_type: DataTypes.ENUM('general', 'stock_specific', 'sector', 'admin', 'premium', 'premium_admin'),
    is_public: DataTypes.BOOLEAN,
    is_premium: DataTypes.BOOLEAN,
    required_plan: DataTypes.STRING,
    admin_only_post: DataTypes.BOOLEAN,
    status: DataTypes.STRING,
    ...
    vip_features_enabled: DataTypes.BOOLEAN
  }),
  ...
  RoomAutomation: createModel('room_automations', {
    ...
    action_type: DataTypes.STRING, // e.g., 'send_message', 'lock_room', 'ban_user'
    config: DataTypes.JSON, // Stores triggers/actions details
    is_active: DataTypes.BOOLEAN,
    created_by: DataTypes.UUID,
    last_triggered_at: DataTypes.DATE
  }),
  ModerationRule: createModel('moderation_rules', {
    chat_room_id: DataTypes.UUID, // Null for global rules
    name: DataTypes.STRING,
    rule_type: DataTypes.STRING, // 'keyword', 'sentiment', 'rate_limit'
    conditions: DataTypes.JSON, // e.g., { keywords: ['bad', 'worse'] }
    action: DataTypes.STRING, // 'block', 'flag', 'mute'
    sensitivity: DataTypes.INTEGER,
    is_active: DataTypes.BOOLEAN,
    created_by: DataTypes.UUID
  }),
  ChatInvite: createModel('chat_invites', {
    chat_room_id: DataTypes.UUID,
    code: DataTypes.STRING,
    created_by: DataTypes.UUID,
    max_uses: DataTypes.INTEGER,
    ...
    expires_at: DataTypes.DATE,
    is_active: DataTypes.BOOLEAN,
    role_to_assign: DataTypes.STRING // Role given to user joining via this invite
  }),
  VIPFeature: createModel('vip_features', {
    chat_room_id: DataTypes.UUID,
    feature_key: DataTypes.STRING, // 'exclusive_polls', 'live_stream', 'priority_support'
