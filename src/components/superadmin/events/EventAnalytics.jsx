...
import { EventTicket, EventCommissionTracking, Event, User, PlatformSetting, EventCheckIn, EventAttendee } from '@/api/entities';
...
import RevenueForecastChart from '../../organizer/RevenueForecastChart';
import ConversionFunnel from '../../organizer/ConversionFunnel';
...
export default function EventAnalytics({ permissions, events: propEvents, tickets: propTickets, attendees: propAttendees, checkIns: propCheckIns = [] }) {
  // Removed internal state for tickets, events, attendees, checkIns as they are now props
  const [commissions, setCommissions] = useState([]);
  const [users, setUsers] = useState([]); // Users are still fetched based on ticket/attendee user_ids
  ...
  // Enriched tickets: This useMemo processes the propTickets to add derived fields
  const enrichedTickets = useMemo(() => {
    if (!propTickets || propTickets.length === 0) return [];
    
    // Create lookup maps from props and fetched users
    const eventsMap = new Map(propEvents.map(event => [event.id, event]));
  ...
  }, [propTickets, propEvents, users, detectAmountField, normalizeAmount, isSuccessfulTicket]);

  // Enriched commissions: This useMemo processes the internally fetched commissions
  ...
  const loadReportsData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch only data not provided as props
      ...
      debug.dataStats = {
        ticketsCount: propTickets.length,
        commissionsCount: commissionsData.length,
        eventsCount: propEvents.length,
        settingsCount: settingsData.length,
        checkInsCount: propCheckIns.length,
        attendeesCount: propAttendees.length
      };

      console.log('📊 RAW DATA LOADED:');
      console.log('- Tickets (from props):', propTickets.length);
      console.log('- Commissions (fetched):', commissionsData.length);
      console.log('- Events (from props):', propEvents.length);
      console.log('- Settings (fetched):', settingsData.length);
      console.log('- Check-Ins (from props):', propCheckIns.length);
      console.log('- Attendees (from props):', propAttendees.length);

      // Log first few ticket records for field inspection from propTickets
      console.log('🎫 FIRST 3 TICKET RECORDS:');
      propTickets.slice(0, 3).forEach((ticket, index) => {
      ...
      // Get unique user IDs from propTickets and propAttendees
      const userIds = [...new Set([
        ...propTickets.map(ticket => ticket.user_id),
        ...propAttendees.map(attendee => attendee.user_id)
      ].filter(Boolean))];
      ...
      // Set internally managed states
      setCommissions(commissionsData);
      setUsers(usersData);
      // propTickets, propEvents, propAttendees, propCheckIns are already set by props

      // Create lookup maps for internal use in this function
      const eventsMap = new Map(propEvents.map(event => [event.id, event]));

      // Calculate gross revenue with detailed logging
      console.log('💰 CALCULATING GROSS REVENUE:');
      // Use the memoized enrichedTickets for calculations
      ...
      // Use the fetched commissionsData for direct calculation
      if (commissionsData.length > 0) {
        ...
        // Calculate on the fly if no commission records exist
      ...
      // Debug field analysis using prop data
      debug.fieldAnalysis = {
        ...
        checkInStatus: {
          total: propCheckIns.length,
          perEvent: propCheckIns.reduce((acc, ci) => {
            const eventTitle = eventsMap.get(ci.event_id)?.title || 'Unknown Event';
            acc[eventTitle] = (acc[eventTitle] || 0) + 1;
            return acc;
          }, {})
        },
        attendeeRSVP: {
          total: propAttendees.length,
          yes: propAttendees.filter(a => a.rsvp_status?.toLowerCase() === 'yes').length,
          no: propAttendees.filter(a => a.rsvp_status?.toLowerCase() === 'no').length,
          maybe: propAttendees.filter(a => a.rsvp_status?.toLowerCase() === 'maybe').length,
          pending: propAttendees.filter(a => !a.rsvp_status || a.rsvp_status?.toLowerCase() === 'pending').length
        }
      };
  ...
  }, [propTickets, propEvents, propAttendees, propCheckIns, detectAmountField, normalizeAmount, isSuccessfulTicket, users, enrichedTickets]);

  useEffect(() => {
    loadReportsData();
  }, [loadReportsData]); // loadReportsData is stable due to useCallback

  // Calculate KPIs using enrichedTickets and enrichedCommissions
  const kpis = useMemo(() => {
    console.log('📊 CALCULATING KPIs FROM STATE...');
    console.log('- Total enriched tickets:', enrichedTickets.length);
    console.log('- Total enriched commissions:', enrichedCommissions.length);

    const successfulTickets = enrichedTickets.filter(ticket => isSuccessfulTicket(ticket));
    ...
    if (enrichedCommissions.length > 0) {
      platformCommissionTotal = enrichedCommissions.reduce((sum, commission) => {
        return sum + (commission.platform_commission || commission.commission_amount || 0);
      }, 0);
    } else {
      // Calculate on the fly if no commission records (or none fetched yet)
    ...
    return finalKPIs;
  }, [enrichedTickets, enrichedCommissions, platformSettings, isSuccessfulTicket]);

  // Filter transactions for the table using enrichedTickets
  const filteredTransactions = useMemo(() => {
    let filtered = enrichedTickets.filter(ticket => 
    ...
    return filtered;
  }, [enrichedTickets, searchTerm, statusFilter, dateFilter]);
  ...
  const handleRefresh = () => {
    // Only refresh internally managed data
    setCommissions([]);
    setUsers([]);
    setPlatformSettings({});
    // Setting isLoading to true will trigger loadReportsData via its useEffect dependency
    setIsLoading(true); 
  ...
  const handleExport = () => {
    const csvContent = [
      ...
      ...filteredTransactions.map(ticket => [
        ...
        ticket.normalizedAmount || 0, // Using normalizedAmount which is the processed value
        ticket.status,
  ...
  return (
    <div className="space-y-6">
      ...
      <RevenueForecastChart 
        ...
        stats={{
          // The outline specified `tickets.reduce((sum, t) => sum + (t.ticket_price || 0), 0)`
          // Using `ticket_price` directly as requested by the outline, assuming it's appropriate for this component.
          totalRevenue: enrichedTickets.reduce((sum, t) => sum + (t.ticket_price || 0), 0)
        }}
      />
        ...
        <CardContent className="text-xs space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-3 rounded">
              ...
              <p className="text-lg">{enrichedTickets.filter(t => isSuccessfulTicket(t)).length}</p>
            </div>
            <div className="bg-white p-3 rounded">
            ...
            <div className="bg-white p-3 rounded">
              ...
              <p className="text-lg">{debugInfo.dataStats?.checkInsCount || 0}</p>
            </div>
            <div className="bg-white p-3 rounded">
              <p className="font-semibold">Total Attendees</p>
              <p className="text-lg">{debugInfo.dataStats?.attendeesCount || 0}</p>
            </div>
            {debugInfo.fieldAnalysis?.attendeeRSVP?.yes > 0 && (
              <div className="bg-white p-3 rounded">
                <p className="font-semibold">Attendees RSVP 'Yes'</p>
                <p className="text-lg">{debugInfo.fieldAnalysis.attendeeRSVP.yes}</p>
              </div>
            )}
          </div>
          ...
          {debugInfo.fieldAnalysis?.statusDistribution && (
            <div className="bg-white p-3 rounded">
              <p className="font-semibold mb-2">Ticket Status Distribution:</p>
          ...
          {debugInfo.fieldAnalysis?.attendeeRSVP && (
            <div className="bg-white p-3 rounded">
              <p className="font-semibold mb-2">Attendee RSVP Distribution:</p>
              <div className="grid grid-cols-3 gap-2 text-xs">
                {Object.entries(debugInfo.fieldAnalysis.attendeeRSVP).map(([status, count]) => (
                  <div key={status} className="flex justify-between">
                    <span>{status}:</span>
                    <span className="font-mono">{count}</span>
                  </div>
