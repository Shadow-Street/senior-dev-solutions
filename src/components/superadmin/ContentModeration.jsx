...
export default function ContentModeration() {
  ...
  const handleStatusUpdate = async (logId, newStatus) => {
    try {
      ...
      const moderatedLog = moderationLogs.find(l => l.id === logId);
      
      // Log audit action
      await logAuditAction(
        currentAdmin,
        AuditActions.CONTENT_MODERATED,
        'ModerationLog',
        logId,
        `Moderated content from user ${moderatedLog?.user?.display_name || 'Unknown'} (ID: ${moderatedLog?.user_id || 'N/A'}) - marked as ${newStatus}. Violation: ${moderatedLog?.violation_type}. Notes: ${adminNotes || 'None'}`
      );
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  const violationConfig = violationTypeConfig[log.violation_type] || {};
