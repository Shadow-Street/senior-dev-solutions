...
import {
  ...
  User,
  PlatformSetting,
  EventCheckIn, // NEW IMPORT
  EventPromoCode, // NEW IMPORT
  EventReminder, // NEW IMPORT
  EventFeedback, // NEW IMPORT
  RefundRequest // NEW IMPORT - assuming this exists in '@/api/entities' or needs to be added
} from '@/api/entities';
...
import ExportManager from './events/ExportManager';
import AdvancedSearchFilter from './events/AdvancedSearchFilter';
import EventManagementTools from './events/EventManagementTools';
...
export default function EventsManagement() {
  ...
  const permissions = useMemo(() => ({
    ...
    isAdmin: ['super_admin', 'admin'].includes(currentUser?.app_role),
    canManageEvents: ['super_admin', 'admin', 'events_manager'].includes(currentUser?.app_role),
    canViewAnalytics: ['super_admin', 'admin', 'events_manager'].includes(currentUser?.app_role),
    canManagePricing: ['super_admin', 'admin'].includes(currentUser?.app_role),
    canApproveEvents: ['super_admin', 'admin', 'events_manager'].includes(currentUser?.app_role),
    canManageRefunds: ['super_admin', 'admin', 'finance_manager'].includes(currentUser?.app_role)
  }), [currentUser]);

  // Renamed from loadAllData to loadData and updated implementation
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      // Batch 1: Core event data
      const [eventsData, ticketsData, attendeesData, userData] = await Promise.all([
        Event.list('-created_date'), // Preserve sorting from original loadAllData
      ...
      // Batch 2: Commission and refunds
      const [commissionData, refundsData] = await Promise.all([
        EventCommissionTracking.list().catch(() => []),
        RefundRequest.filter({ transaction_type: 'event_ticket' }).catch(() => [])
      ]);

      setCommissionTracking(commissionData);
      setRefundRequests(refundsData);

      // Add delay
      await new Promise(resolve => setTimeout(resolve, 1000));

      // Batch 3: New features data (check-ins, promo codes, etc.)
      const [checkInsData, promoCodesData, remindersData, feedbacksData] = await Promise.all([
        EventCheckIn.list().catch(() => []),
        EventPromoCode.list().catch(() => []),
        EventReminder.list().catch(() => []),
        EventFeedback.list().catch(() => [])
      ]);
  ...
  }, []); // Dependencies remain empty as before for initial load
  ...
  const handleCancelSuccess = async () => {
    ...
    await loadData(); // Call loadData after successful cancellation
  };
  ...
  const commonProps = {
    ...
    platformSettings,
    permissions,
    onUpdate: loadData, // Updated to loadData
    currentUser,
    isLoading,
    ...
    // NEW: Pass additional data to common props
    checkIns,
    promoCodes,
    reminders,
    feedbacks,
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="mt-6">
          <TabsContent value="overview">
            <EventsOverview
              ...
              feedbacks={feedbacks}
              onSelectAll={() => handleSelectAll(filteredEvents)}
            />
          </TabsContent>
          ...
          <TabsContent value="tools">
            <EventManagementTools
              {...commonProps} // Include commonProps for consistency, though outline specified direct props
              events={filteredEvents} // Still pass filtered events if tools operate on them
              checkIns={checkIns}
              promoCodes={promoCodes}
              reminders={reminders}
              feedbacks={feedbacks}
              tickets={tickets} // Already part of commonProps, but good to be explicit if needed
            />
          </TabsContent>
          {/* No TabsContent for 'capacity', 'export', 'search', 'refunds' as per outline,
              as these might be handled by modals or integrated into other tabs implicitly. */}
        </div>
      </Tabs>
      ...
      <ExportManager
        ...
        checkIns={checkIns} // NEW: Pass checkIns
        promoCodes={promoCodes} // NEW: Pass promoCodes
        reminders={reminders} // NEW: Pass reminders
        feedbacks={feedbacks} // NEW: Pass feedbacks
        refundRequests={refundRequests} // NEW: Pass refundRequests
