...
import { PledgeSession, Pledge, PledgePayment, PledgeExecutionRecord, PledgeAuditLog, PledgeAccessRequest, Stock, User } from '@/lib/apiClient'; // Added Stock, User
...
// ✅ NEW: Delay helper function to space out API calls
const delay = ms => new Promise(res => setTimeout(res, ms));

export default function MyPledgePortfolio({ user }) {
  // MODIFIED: Initial active tab
  ...
  const [data, setData] = useState({
    sessions: [], // Replaces availableSessions
    pledges: [], // Replaces userPledges (will be filtered for active/executed)
    executions: [], // Replaces executionRecords
    stockPrices: {} // Replaces stockPrices
  });

  // Kept separate as per outline structure
  ...
  const [userDematAccountId, setUserDematAccountId] = useState(null); // New state to store demat ID

  // Ref to track if the component is mounted, for `handleRefresh`
  const isMountedRef = useRef(true);
  // ✅ NEW: Ref to prevent multiple rapid clicks
  const toggleButtonRef = useRef(false);

  // ✅ MODIFIED: useRealTimePledgeUpdates no longer supplies 'sessions' directly to this component.
  // The component now manages 'availableSessions' state itself.
  // The hook might still use or process sessions internally, but it's not exposed here directly anymore.
  const {
    pledgeStats,
    executionUpdates,
    lastUpdate,
    ...
    resumePolling,
    enablePolling
  } = useRealTimePledgeUpdates(user?.id, [], 30000); // Pass empty array for initialPledges, as availableSessions are loaded locally
  ...
  // ✅ NEW: loadData function to fetch all relevant data
  const loadData = useCallback(async () => {
    if (!user?.id || !isMountedRef.current) return;

    setIsLoading(true);
    console.log('🔄 Loading all pledge portfolio data...');
    ...
    try {
      ...
      const [
        ...
        PledgeSession.filter({ status: { '$in': ['active', 'closed', 'executing', 'awaiting_sell_execution'] } }, '-session_start').catch(err => {
          console.error("Failed to load sessions:", err);
          return [];
        }),
        // Fetch user's pledges
        Pledge.filter({ user_id: user.id }, '-created_date').catch(err => {
          console.error("Failed to load pledges:", err);
          return [];
        }),
        // Fetch user's execution records
        PledgeExecutionRecord.filter({ user_id: user.id }, '-executed_at').catch(err => {
          console.error("Failed to load execution records:", err);
          return [];
        }),
        // Fetch user's payment history
        PledgePayment.filter({ user_id: user.id }, '-created_date').catch(err => {
          console.error("Failed to load payment history:", err);
          return [];
        }),
        // Fetch user's audit logs
        PledgeAuditLog.filter({ actor_id: user.id }, '-created_date', 50).catch(err => {
          console.error("Failed to load audit logs:", err);
          return [];
        }),
        // Fetch user's pledge access requests
        PledgeAccessRequest.filter({ user_id: user.id }, '-created_date').catch(err => {
      ...
      let fetchedStockPrices = {};
      if (stockSymbols.length > 0) {
        try {
          const { stockAPI } = await import('../stocks/LiveStockAPI'); // Import LiveStockAPI
            ...
            try {
              ...
              if (stockData) {
                fetchedStockPrices[symbol] = {
                  ...
                  change: stockData.change_percent || 0
                };
              }
            } catch (error) {
              console.warn(`Failed to fetch live price for ${symbol}:`, error);
              // Fallback to dummy data if live API fails
          ...
          console.error("Failed to load LiveStockAPI or fetch stock prices:", stockApiError);
        }
      }
      ...
      console.log('✅ All pledge portfolio data loaded successfully.');
    } catch (error) {
  ...
  // ✅ MODIFIED: This is the critical fix. Data is now loaded via a single function.
  useEffect(() => {
    const initialize = async () => {
      if (user?.id) {
        await loadData();
        // The hook's refresh also makes API calls, so it runs last
        await refresh(); // Refresh real-time updates as well after initial data load
      }
    };
    initialize();
  }, [loadData, refresh, user?.id]); // Dependencies updated to reflect consolidated loading

  useEffect(() => {
    // ✅ FIX: Use lastUpdate timestamp as a reliable trigger for re-loading data.
    // This ensures that any change detected by the polling hook triggers a refresh.
    if (lastUpdate > 0) {
  ...
  // ✅ MODIFIED: Refresh function also uses sequential loading now
  const handleRefresh = async () => {
    console.log('🔄 Manual refresh triggered');
    setIsLoading(true);
    try {
      await loadData();
      await refresh(); // Also refresh real-time updates
  ...
  const handleToggleLiveUpdates = useCallback(() => {
    ...
    toast.dismiss('live-updates-toggle');
  ...
  }, [isPolling, pausePolling, enablePolling]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        {/* MODIFIED: Tab value from "pledge" to "active" */}
        ...
        {/* MODIFIED: Tab value from "committed" to "my-pledges" */}
