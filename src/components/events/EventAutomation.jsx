 ...
 * - Update event ratings from reviews
 */
export const useEventAutomation = () => {
    ...
    try {
      ...
      for (const event of events) {
        ...
        if (eventDate < now && event.status !== 'completed') {
          // Update event rating before marking as completed
  ...
  // NEW: Update event rating from reviews
  const updateEventRating = async (eventId) => {
    try {
      const reviews = await EventReview.filter({
        event_id: eventId,
        status: 'approved'
      });
