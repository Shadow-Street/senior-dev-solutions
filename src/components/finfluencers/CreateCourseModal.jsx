...
export default function CreateCourseModal({ open, onClose, onCreate, editingCourse }) {
  ...
  useEffect(() => {
    if (editingCourse) {
      console.log('📝 Loading course for editing:', editingCourse);
      
      // Convert curriculum array to string
      const curriculumString = Array.isArray(editingCourse.curriculum) 
        ? editingCourse.curriculum.join('\n')
        : (editingCourse.curriculum || '');

      // Format scheduled_date for datetime-local input
      const scheduledDate = editingCourse.scheduled_date 
        ? new Date(editingCourse.scheduled_date).toISOString().slice(0, 16)
        : '';
      ...
      setFormData({
        ...
        duration_hours: editingCourse.duration_hours?.toString() || '',
        ...
        course_content_url: editingCourse.course_content_url || '',
        max_participants: editingCourse.max_participants?.toString() || '',
  ...
  const handleSubmit = (e) => {
    ...
    // Don't reset form here - let parent component handle it
  };
