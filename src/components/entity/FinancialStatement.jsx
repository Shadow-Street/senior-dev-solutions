...
export default function FinancialStatement({ entityType, entityId, entityName }) {
  ...
  const loadStatementData = async () => {
    ...
    try {
      ...
      let earnings = [];
      let commissions = []; // This variable is not used anywhere, can be removed if not planned for future
      let payouts = [];
      let grossRevenue = 0;
      let platformCommission = 0;
      let netEarnings = 0;

      if (entityType === 'event_organizer') {
        // Fetch event commission tracking
        ...
        earnings = filteredEventCommissions.map(c => ({
          date: c.created_date,
        ...
        // Fetch payout requests specifically for event_organizer
        const allPayouts = await PayoutRequest.filter({ 
          entity_id: entityId,
          entity_type: 'event_organizer' 
        }).catch(() => []);
        ...
        setIsLoading(false);
        return; // Exit function after handling event_organizer specific logic
      }
      ...
      if (entityType === 'finfluencer') {
        ...
        // Fetch event commission earnings - Note: This assumes a finfluencer can also be an event organizer.
        // If 'event_organizer' is a completely separate entity type, this block might need removal.
        const eventCommissions = await EventCommissionTracking.filter({
          organizer_id: entityId
        }).catch(() => []);
      ...
      // Fetch payout requests (this block will only run for finfluencer, advisor, vendor)
      const allPayouts = await PayoutRequest.filter({ 
        entity_id: entityId,
